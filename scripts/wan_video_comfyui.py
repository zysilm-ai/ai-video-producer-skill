...
I2V_WORKFLOW_MOE_FAST = WORKFLOW_DIR / "wan_i2v_moe_fast.json"  # WAN 2.2 MoE + LightX2V LoRA + ALG (8 steps, enhanced motion)
...
def generate_video(
    ...
    if workflow_path:
        ...
        if use_moe_fast:
            ...
            print_status("Mode: Image-to-Video (I2V) - WAN 2.2 MoE + ALG (8 steps, enhanced motion)")
        elif use_moe:
    ...
    if not use_q6k and not use_moe and not use_moe_fast:
...
def main():
    ...
    parser.add_argument(
        "--moe-fast",
        action="store_true",
        help="Use WAN 2.2 MoE + ALG (8 steps, enhanced motion dynamics) - RECOMMENDED"
    )
    parser.add_argument(
        "--workflow",
